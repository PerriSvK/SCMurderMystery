...
public class Game
{
    ...
    public void start(boolean force)
    {
        ...
        task = Bukkit.getScheduler().runTaskTimer(Main.get(), () ->
        {
            ...
            if(countdown <= 5)
            {
                ...
                switch (countdown)
                {
                    case 5: cc = ChatColor.GREEN; break;
                    case 4: cc = ChatColor.YELLOW; break;
                    case 3: cc = ChatColor.GOLD; break;
                    case 2: cc = ChatColor.RED; break;
                    default: cc = ChatColor.DARK_RED;
                }
    ...
    private void loop()
    {
        ...
        {
            alive.get(i).getPlayer().teleport(spawn.get(ik.get(i)));
            alive.get(i).getPlayer().getInventory().clear();
            alive.get(i).setType(PlayerType.Innocent);
        }
    ...
    private void gameOver(GameOverReason reason)
    {
        ...
        switch (reason)
        {
            case ALL_DEAD: s += Lang.KILLER;
                Main.get().getServer().getOnlinePlayers().forEach(p ->
                        p.sendTitle(Lang.LOOSE_MORE, Lang.I_LOOSE_REASON, 10, 80, 10));
                        killer.getPlayer().sendTitle(Lang.WIN, Lang.KILLER_WIN_REASON, 10, 80, 10); break;
            default: s += Lang.DETECTIVE;
                Main.get().getServer().getOnlinePlayers().forEach(p ->
